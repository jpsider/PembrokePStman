...
Describe "Invoke-ReviewAssignedTaskSet function for $moduleName" {
    ...
    It "Should not Throw if there are no assigned tasks." {
        $MAX_CONCURRENT_TASKS = 1
        Mock -CommandName 'Test-Connection' -MockWith {
            $true
        }
        ...
        Mock -CommandName 'Write-LogLevel' -MockWith {}
        {Invoke-ReviewAssignedTaskSet -RestServer localhost -TableName tasks -MAX_CONCURRENT_TASKS $MAX_CONCURRENT_TASKS} | Should -not -Throw
        ...
        Assert-MockCalled -CommandName 'Write-LogLevel' -Times 10 -Exactly
    }
}