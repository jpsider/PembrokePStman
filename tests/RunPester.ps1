...
Set-Location C:\OPEN_PROJECTS\ProjectPembroke\PembrokePSwman
...
foreach ($Directory in $Directories)
{
    ...
    $files = Get-ChildItem .\PembrokePSwman\$Directory
    foreach ($file in $files)
    {
        # Source the file
        $FileName = $file.Name
        Write-Output "Importing file $FileName"
        . .\PembrokePSwman\$Directory\$FileName
    }
    # Execute Pester for the Directory.
    $results = Invoke-pester .\tests\$Directory\*.ps1 -CodeCoverage .\PembrokePSwman\$Directory\*.ps1 -PassThru
